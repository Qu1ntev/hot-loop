...
use std::sync::Arc;
use crate::{ModelWeights, Session};
...
impl LayerWeights {
    ...
    fn forward(&self, x: &Tensor, mask: Option<&Tensor>, offset: usize, kv_cache: &mut ConcatKvCache) -> Result<Tensor> {
...
impl Qwen3 {
        ...
        T: AsRef<[u8]>,
    {
        ...
        let tokenizer = Tokenizer::from_bytes(tokenizer).unwrap(); // FIX!!!
    ...
    // pub fn create_kv_cache(&self) -> Vec<ConcatKvCache> {
    //     let mut kv_cache = Vec::with_capacity(self.layers.len());
    //
    //     for _ in 0..self.layers.len() {
    //         kv_cache.push(ConcatKvCache::new(2));
    //     }
    //
    //     kv_cache
    // }
...
impl ModelWeights for Qwen3 {
    fn forward(&self, input: &Tensor, offset: usize, kv_cache: &mut Vec<ConcatKvCache>) -> Result<Tensor> {
        let (b, l) = input.dims2()?;
        let mut h = self.embed_tokens.forward(input)?;
        let causal_mask = if l == 1 {
            None
