...
//     }
// 
//     fn forward(&self, x: &Tensor, mask: Option<&Tensor>, offset: usize, kv_cache: &mut KvCache) -> Result<Tensor> {
...
//         device: &Device,
//     ) -> Result<Self> {
//         let ct = gguf_file::Content::read(reader)?;
// 
...
//     }
// 
//     pub fn forward(&self, input: &Tensor, offset: usize, kv_cache: &mut KvCache) -> Result<Tensor> { // Start
//         let (b, l) = input.dims2()?;
...
//             h = layer.forward(&h, causal_mask.as_ref(), offset, kv_cache)?;
//         }
